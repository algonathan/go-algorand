...
func TestTxIDAndGroupIDCalculation(t *testing.T) {
	...
	type expectedInfo struct {
		...
		expectedGroup cryptbase.Digest
	}
	...
	verifyTxIDs := func(t *testing.T, unified bool, actual [11]actualInfo) {
		...
		var gcAABgroup, gcACDgroup, gcBABgroup, gcBCDgroup cryptbase.Digest
		...
		if unified {
			...
			gcAABgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildAAIndex].txn).InnerID(childAtxid, 0)),
					cryptbase.Digest(withoutGroupID(actual[grandchildABIndex].txn).InnerID(childAtxid, 1)),
				},
			})
			require.Equal(t, cryptbase.Digest{0xb4, 0x0, 0x71, 0x2a, 0xee, 0x7f, 0x4, 0x20, 0x55, 0xf8, 0xf9, 0x8f, 0xaf, 0x12, 0x82, 0x57, 0x2, 0x45, 0xe9, 0x16, 0x45, 0x22, 0xc3, 0x22, 0xfb, 0x1a, 0x23, 0x4d, 0x78, 0xfe, 0xa1, 0x42}, gcAABgroup)
			...
			gcACDgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildACIndex].txn).InnerID(childAtxid, 2)),
					cryptbase.Digest(withoutGroupID(actual[grandchildADIndex].txn).InnerID(childAtxid, 3)),
				},
			})
			require.Equal(t, cryptbase.Digest{0x45, 0xce, 0x49, 0xe6, 0xa6, 0xbe, 0xa7, 0x4d, 0xb1, 0x66, 0x5d, 0xaa, 0xf6, 0xf0, 0xda, 0x78, 0x77, 0x3d, 0x6f, 0x97, 0x65, 0xd7, 0x27, 0x1, 0x82, 0x6a, 0x2c, 0xe0, 0x4c, 0xd8, 0x3b, 0x2}, gcACDgroup)
			...
			gcBABgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildBAIndex].txn).InnerID(childBtxid, 0)),
					cryptbase.Digest(withoutGroupID(actual[grandchildBBIndex].txn).InnerID(childBtxid, 1)),
				},
			})
			require.Equal(t, cryptbase.Digest{0x48, 0x7c, 0x9, 0x76, 0xbc, 0x43, 0x65, 0x7a, 0x1d, 0xdc, 0xfb, 0x68, 0x47, 0x12, 0x8b, 0x80, 0xd2, 0xdd, 0xff, 0x22, 0x1b, 0xe1, 0x89, 0xcc, 0xb5, 0xb3, 0x94, 0xa4, 0x49, 0x63, 0xd8, 0x10}, gcBABgroup)
			...
			gcBCDgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildBCIndex].txn).InnerID(childBtxid, 2)),
					cryptbase.Digest(withoutGroupID(actual[grandchildBDIndex].txn).InnerID(childBtxid, 3)),
				},
			})
			require.Equal(t, cryptbase.Digest{0x96, 0x90, 0x1, 0x64, 0x24, 0xa5, 0xda, 0x4, 0x3d, 0xd, 0x40, 0xc9, 0xf6, 0xfa, 0xc3, 0xa6, 0x26, 0x19, 0xd3, 0xf0, 0xb7, 0x28, 0x87, 0xf8, 0x5a, 0xd1, 0xa7, 0xbc, 0x1d, 0xad, 0x8b, 0xfc}, gcBCDgroup)
		} else {
			...
			gcAABgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildAAIndex].txn).InnerID(childAtxn.ID(), 0)),
					cryptbase.Digest(withoutGroupID(actual[grandchildABIndex].txn).InnerID(childAtxn.ID(), 0)),
				},
			})
			require.Equal(t, cryptbase.Digest{0xbc, 0xd8, 0xa1, 0xff, 0x96, 0xd4, 0x2b, 0x39, 0xc3, 0x1e, 0x9b, 0x26, 0xf3, 0xc, 0x78, 0x18, 0x83, 0x40, 0xe0, 0xf0, 0xa5, 0xeb, 0xc3, 0x63, 0xc8, 0xac, 0xec, 0xdb, 0x1, 0x36, 0xf9, 0xa}, gcAABgroup)
			...
			gcACDgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildACIndex].txn).InnerID(childAtxn.ID(), 2)),
					cryptbase.Digest(withoutGroupID(actual[grandchildADIndex].txn).InnerID(childAtxn.ID(), 2)),
				},
			})
			require.Equal(t, cryptbase.Digest{0x72, 0x6d, 0xad, 0x41, 0x1c, 0x6b, 0x71, 0x1d, 0x4a, 0xe8, 0xf8, 0x82, 0x7, 0xf6, 0x37, 0xb9, 0x5d, 0x80, 0xc7, 0xf8, 0x86, 0x2d, 0xd5, 0xa1, 0x16, 0x29, 0x77, 0xc9, 0x44, 0xf7, 0x0, 0xb2}, gcACDgroup)
			...
			gcBABgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildBAIndex].txn).InnerID(childBtxn.ID(), 0)),
					cryptbase.Digest(withoutGroupID(actual[grandchildBBIndex].txn).InnerID(childBtxn.ID(), 0)),
				},
			})
			require.Equal(t, cryptbase.Digest{0xbc, 0xd8, 0xa1, 0xff, 0x96, 0xd4, 0x2b, 0x39, 0xc3, 0x1e, 0x9b, 0x26, 0xf3, 0xc, 0x78, 0x18, 0x83, 0x40, 0xe0, 0xf0, 0xa5, 0xeb, 0xc3, 0x63, 0xc8, 0xac, 0xec, 0xdb, 0x1, 0x36, 0xf9, 0xa}, gcBABgroup)
			...
			gcBCDgroup = cryptbase.HashObj(transactions.TxGroup{
				TxGroupHashes: []cryptbase.Digest{
					cryptbase.Digest(withoutGroupID(actual[grandchildBCIndex].txn).InnerID(childBtxn.ID(), 2)),
					cryptbase.Digest(withoutGroupID(actual[grandchildBDIndex].txn).InnerID(childBtxn.ID(), 2)),
				},
			})
			require.Equal(t, cryptbase.Digest{0x72, 0x6d, 0xad, 0x41, 0x1c, 0x6b, 0x71, 0x1d, 0x4a, 0xe8, 0xf8, 0x82, 0x7, 0xf6, 0x37, 0xb9, 0x5d, 0x80, 0xc7, 0xf8, 0x86, 0x2d, 0xd5, 0xa1, 0x16, 0x29, 0x77, 0xc9, 0x44, 0xf7, 0x0, 0xb2}, gcBCDgroup)
		}
